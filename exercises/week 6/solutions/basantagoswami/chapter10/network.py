...
class NetworkEnvelope:
    ...
    def parse(cls, s, testnet=False):
        ...
        if hash256(payload)[:4] != checksum:
            raise IOError('Invalid checksum')
    ...
    def serialize(self):
        ...
        # payload length 4 bytes, little endian
        payload_lenght = int_to_little_endian(len(self.payload), 4)
        ...
        return magic + command + payload_lenght + checksum + payload
...
class VersionMessage:
    ...
    def serialize(self):
        ...
        services = int_to_little_endian(self.services, 8)
        
        # timestamp is 8 bytes little endian
        timestamp = int_to_little_endian(self.timestamp, 8)
        
        # receiver services is 8 bytes little endian
        receiver_services = int_to_little_endian(self.receiver_services, 8)
        
        # IPV4 is 10 00 bytes and 2 ff bytes then receiver ip
        receiver_ip = b'\x00' * 10 + b'\xff\xff' + self.receiver_ip
        
        # receiver port is 2 bytes, big endian
        receiver_port = self.receiver_port.to_bytes(2, 'big')
        
        # sender services is 8 bytes little endian
        sender_services = int_to_little_endian(self.sender_services, 8)
        
        # IPV4 is 10 00 bytes and 2 ff bytes then sender ip
        sender_ip = b'\x00' * 10 + b'\xff\xff' + self.sender_ip
        
        # sender port is 2 bytes, big endian
        sender_port = self.sender_port.to_bytes(2, 'big')
        
        # nonce should be 8 bytes
        nonce = self.nonce
        
        # useragent is a variable string, so varint first
        user_agent_varint = encode_varint(len(self.user_agent))
        user_agent = self.user_agent
        
        # latest block is 4 bytes little endian
        latest_block = int_to_little_endian(self.latest_block, 4)
        
        # relay is 00 if false, 01 if true
        relay = b'\x01' if self.relay else b'\x00'
        
        return version + services + timestamp + receiver_services + receiver_ip + receiver_port + sender_services + sender_ip + sender_port + nonce + user_agent_varint + user_agent + latest_block + relay
...
class GetHeadersMessage:
    ...
    def serialize(self):
        ...
        hash_count = encode_varint(self.num_hashes)
        ...
        return version + hash_count + start_block + end_block
