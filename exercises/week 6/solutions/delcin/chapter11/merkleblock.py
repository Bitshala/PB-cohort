...
class MerkleBlock:
    ...
    def parse(cls, s):
        ...
        total_transactions = little_endian_to_int(s.read(4))
        ...
        flag = s.read(flag_len)
        # initialize class
        return MerkleBlock(version, prev_block, m_root, timestamp, bits, nonce, total_transactions, hashes, flag)
    ...
    def is_valid(self):
        ...
        hashes = list(map(lambda x: x[::-1], self.hashes))
        mt = MerkleTree(self.total)
        mt.populate_tree(bits, hashes)
        return mt.root()[::-1] == self.merkle_root
