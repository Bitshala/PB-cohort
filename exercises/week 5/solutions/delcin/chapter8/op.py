...
def op_checkmultisig(stack, z):
    ...
    try:
        ...
        points = list(map(S256Point.parse, sec_pubkeys))
        # parse all the signatures
        sigs = list(map(Signature.parse, der_signatures))
        i = 0; j = 0

        # Based on this reference -> https://en.bitcoin.it/wiki/OP_CHECKMULTISIG
        while i < n and j < m:
            if points[i].verify(z, sigs[j]):
                j += 1
            i += 1
