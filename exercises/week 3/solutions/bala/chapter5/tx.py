...
class Tx:
    ...
    def fee(self):
        ...
        ninputs=len(self.tx_ins)
        for i in range(ninputs):
            g = TxIn(self.tx_ins[i].prev_tx, self.tx_ins[i].prev_index)
            isum+=g.value()
        # use TxOut.amount to sum up the output amounts
        noutputs=len(self.tx_outs)
        for j in range(noutputs):
            osum+=self.tx_outs[j].amount
