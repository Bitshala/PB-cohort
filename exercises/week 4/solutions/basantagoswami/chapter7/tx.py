...
class Tx:
    ...
    def sig_hash(self, input_index):
        ...
        serialized_tx = int_to_little_endian(self.version, 4)        
        
        # add how many inputs there are using encode_varint
        serialized_tx += encode_varint(len(self.tx_ins))
        ...
        serialized_tx += encode_varint(len(self.tx_outs))
        
        # add the serialization of each output
        for tx_out in self.tx_outs:
            serialized_tx += tx_out.serialize()
            
        # add the locktime using int_to_little_endian in 4 bytes
        serialized_tx += int_to_little_endian(self.locktime, 4)
        
        # add SIGHASH_ALL using int_to_little_endian in 4 bytes
        serialized_tx += int_to_little_endian(SIGHASH_ALL, 4)
        
        # hash256 the serialization
        hashed_tx = hash256(serialized_tx)
        
        # convert the result to an integer using int.from_bytes(x, 'big')
        return int.from_bytes(hashed_tx, 'big')
